...
React-created orders appear in **WooCommerce > Orders** with normal WooCommerce line items, customer billing/shipping details, order notes, and a Zyra Luxe React order panel. Existing orders are unchanged; install plugin version `1.0.5` before testing a new checkout. Headless orders remain payable without a WordPress browser session and are securely returned to the matching account by billing email. The selected carrier is saved as a WooCommerce shipping line, and the payment method is saved as Razorpay. This version also handles authenticated CORS preflight requests.
